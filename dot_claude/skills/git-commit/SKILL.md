...
- Co-authors: `Co-authored-by: Name <email>` (only when collaborating with actual people)

**IMPORTANT**: Never add footers indicating the commit message was generated by Claude Code, AI, or automated tools. Do not add signatures like "Generated with Claude Code" or "Co-authored-by: Claude Sonnet" under any circumstances.
...
If the user approves, execute the commit using the user's default git configuration:
...
**Note**: The commit will use the user's configured git identity (from `git config user.name` and `git config user.email`). Do not override the author or add co-author information for AI assistance.
...
- **Never** add footers indicating AI assistance (like "🤖 Generated with Claude Code" or "Co-authored-by: Claude Sonnet"). The commit should appear as the user's own work
- Use the user's default git identity—never override author information or add AI co-author credits
